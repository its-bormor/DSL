...
def build_dashboard_embed(filter_type: str) -> discord.Embed:
    ...
    label = stats["label"]
    active = stats["active_doctors"]
    hours = stats["total_hours"]
    shifts = stats["total_shifts"]
    unique = stats["unique_doctors"]
    ...
    if not top_docs:
        ...
        for idx, doc in enumerate(top_docs, 1):
            ...
            doc_hours = doc['total_hours']
            count = doc['shift_count']
...
class ShiftPanelView(discord.ui.View):
    ...
    async def check_out_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            check_in_time = shift["check_in"]
            check_out_time = shift["check_out"]
...
async def setup_panel(interaction: discord.Interaction):
    ...
    await interaction.followup.send(
        ...
        "• บอร์ดจะทำการอัปเดตรายชื่ออัติโนมัติเมื่อแพทย์ทำการเข้า-ออกเวร",
        ephemeral=True
    )
...
async def my_shifts(interaction: discord.Interaction):
    ...
    for shift in history:
        ...
        check_out = shift['check_out'] if shift['check_out'] else "*กำลังเข้าเวรอยู่*"
        duration = f"{shift['duration_hours']} ชั่วโมง" if shift['duration_hours'] is not None else "-"
